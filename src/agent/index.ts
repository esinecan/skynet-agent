...
export async function processQueryStream(query: string, threadId = "default"): Promise<ReadableStream> {
  ...
  try {
    ...
    logger.debug('Initial state prepared for streaming workflow', {
      threadId,
    ...
    logger.debug('Streaming query processing completed successfully', {
      threadId,
      totalTimeMs: totalTime,
      workflowTimeMs: workflowTime,
      responseLength: response.length,
      finalMessageCount: resultState.messages?.length || 0,
      streamConversionTimeMs: totalTime - workflowTime,
      efficiency: {
    ...
    logger.debug('Streaming query processing failed with error', {
      threadId,
      totalTimeMs: totalTime,
      queryLength: query?.length || 0,
      errorType: err.constructor.name,
