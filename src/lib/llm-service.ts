...
- WORKS_ON: When a person is involved in or assigned to a project (e.g., (Person:JohnDoe)-[:WORKS_ON]->(Project:SkynetAgent))
- USES: When a person or project utilizes a specific tool or technology (e.g., (Person:User)-[:USES]->(Tool:VSCode))
- PREFERS: For capturing preferences about tools, methods, or styles (e.g., (User)-[:PREFERS]->(Approach:FunctionalComponents))
- LEARNED_ABOUT: To track knowledge acquisition or understanding (e.g., (User)-[:LEARNED_ABOUT]->(Concept:GraphDatabases))
- HAS_PROPERTY: For dynamic attributes or states (e.g., (Task:Deployment)-[:HAS_PROPERTY]->(Status:InProgress))
TEXT TO ANALYZE:
${text}`;
